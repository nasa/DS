...
CFE_Status_t DS_TableInit(void)
{
    ...
    */
    Result1 = CFE_TBL_Register(&DS_AppData.DestFileTblHandle,
                               DS_DESTINATION_TBL_NAME,
                               sizeof(DS_DestFileTable_t),
                               TableRegisterFlags,
    ...
    if (Result1 == CFE_TBL_INFO_RECOVERED_TBL)
    {
        ...
        */
        ...
        CFE_EVS_SendEvent(DS_INIT_TBL_CDS_EID,
    ...
    else
    {
        ...
        */
        CFE_EVS_SendEvent(DS_INIT_TBL_ERR_EID,
    ...
    {
        Result1 = CFE_TBL_Register(&DS_AppData.FilterTblHandle,
                                   DS_FILTER_TBL_NAME,
                                   sizeof(DS_FilterTable_t),
                                   TableRegisterFlags,
...
CFE_Status_t DS_TableVerifyDestFile(const void *TableData)
{
    ...
    */
    CFE_EVS_SendEvent(DS_FIL_TBL_EID,
...
bool DS_TableVerifyDestFileEntry(DS_DestFileEntry_t *DestFileEntry, uint8 TableIndex, int32 ErrorCount)
{
    ...
    */
    if (DS_TableVerifyType(DestFileEntry->FileNameType) == false)
    {
        if (ErrorCount == 0)
        {
            CFE_EVS_SendEvent(DS_FIL_TBL_ERR_EID,
                              CFE_EVS_EventType_ERROR,
                              "%s index = %d, filename type = %d",
                              CommonErrorText,
                              TableIndex,
                              DestFileEntry->FileNameType);
        }
        Result = false;
    }
    ...
    else if (DS_TableVerifyCount(DestFileEntry->SequenceCount) == false)
    {
        if (ErrorCount == 0)
        {
            CFE_EVS_SendEvent(DS_FIL_TBL_ERR_EID,
                              ...
                              CommonErrorText,
                              (int)TableIndex,
                              (int)DestFileEntry->SequenceCount);
        }
        Result = false;
    }
...
CFE_Status_t DS_TableVerifyFilter(const void *TableData)
{
    ...
    */
    for (i = 0; i < DS_PACKETS_IN_FILTER_TABLE; i++)
    {
        ...
        else
        {
            for (j = 0; j < i; j++)
            {
                if (CFE_SB_MsgId_Equal(FilterTable->Packet[i].MessageID, FilterTable->Packet[j].MessageID))
                {
                    break;
                }
            }
            ...
            if (j < i)
            {
                if (CountBad == 0)
                {
                    CFE_EVS_SendEvent(DS_FLT_TBL_ERR_EID,
                                      CFE_EVS_EventType_ERROR,
                                      "Filter table verify err: MID = 0x%08lX, index = %d, duplicates index = %d",
    ...
    ** Note that totals include each table entry plus the descriptor
    */
    CFE_EVS_SendEvent(DS_FLT_TBL_EID,
                      CFE_EVS_EventType_INFORMATION,
    ...
    for (i = 0; (i < DS_FILTERS_PER_PACKET) && (Result == true); i++)
    {
        ...
        if (DS_TableEntryUnused(FilterParms, sizeof(DS_FilterParms_t)) == false)
        {
            ...
            */
            if (DS_TableVerifyFileIndex((uint32)FilterParms->FileTableIndex) == false)
            {
                if (ErrorCount == 0)
                {
                    CFE_EVS_SendEvent(DS_FLT_TBL_ERR_EID,
                                      ...
                                      CommonErrorText,
                                      (unsigned long)CFE_SB_MsgIdToValue(PacketEntry->MessageID),
                                      (int)TableIndex,
                                      (int)i,
            ...
            else if (DS_TableVerifyParms(FilterParms->Algorithm_N, FilterParms->Algorithm_X, FilterParms->Algorithm_O)
                     == false)
            {
                if (ErrorCount == 0)
                {
    ...
    */
    ...
    if (Result != CFE_SUCCESS)
    {
        ...
        */
        ...
        CFE_EVS_SendEvent(DS_INIT_CDS_ERR_EID,
                          CFE_EVS_EventType_ERROR,
