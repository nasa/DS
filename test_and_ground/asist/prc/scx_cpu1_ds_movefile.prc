...
ut_setupevents "SCX", "CPU1", "FM", FM_STARTUP_EID, "INFO", 2
...
/SCX_CPU1_TO_ADDPACKET STREAM=fmPkt PKT_SIZE=X'0' PRIORITY=X'0' RELIABILITY=X'0' BUFLIMIT=x'4'
...
write ";***********************************************************************"
local moveDirName = SCX_CPU1_DS_DF_TBL[seqFileEntry].MoveDirName
local pathName = SCX_CPU1_DS_DF_TBL[seqFileEntry].PathName

write "==> Creating File Move Directory =  ", moveDirName

/SCX_CPU1_FM_DirCreate DirName=moveDirName
wait 5
...
;; 4 writes per file
local expFileWrites = SCX_CPU1_DS_FileWriteCnt + 16
...
local expHdrUpdates = SCX_CPU1_DS_FileUpdCnt + 4
...
if (SCX_CPU1_DS_FileState[seqFileEntry].FileName <> "") then
  ...
  /SCX_CPU1_FM_DirListTlm DirName=pathName Offset=0 GetStatFlag=0
  wait 10

  ;; Parse the Directory listing packet for fileName1
  for i = 1 to SCX_CPU1_FM_PktFiles do
    if (fileName1 = SCX_CPU1_FM_DirList[i].name) then
  ...
  /SCX_CPU1_FM_DirListTlm DirName=moveDirName Offset=0 GetStatFlag=0
  wait 10
...
if (SCX_CPU1_DS_FileState[seqFileEntry].FileName <> "") then
  ...
  ;; Parse the Directory listing packet for fileName1
  for i = 1 to SCX_CPU1_FM_PktFiles do
    if (fileName2 = SCX_CPU1_FM_DirList[i].name) then
...
local fileAge = SCX_CPU1_DS_DF_TBL[seqFileEntry].FileAge
local reportedAge = SCX_CPU1_DS_FileState[seqFileEntry].FileAge
...
while (waitTime > 0) do
  ...
  reportedAge = SCX_CPU1_DS_FileState[seqFileEntry].FileAge
  ...
  if (SCX_CPU1_DS_FileState[seqFileEntry].FileName = "") AND ;;
     (SCX_CPU1_DS_FileState[seqFileEntry].FileAge = 0) then
    waitTime = 0
...
;; Parse the Directory listing packet for fileName1
for i = 1 to SCX_CPU1_FM_PktFiles do
  if (fileName3 = SCX_CPU1_FM_DirList[i].name) then
...
write ";*********************************************************************"
write ";  End procedure SCX_CPU1_ds_movefile"
write ";*********************************************************************"
ENDPROC
