...
write ";***********************************************************************"
write ";  Step 1.2: Upload the initial DS table load images to CPU1."
write ";***********************************************************************"
...
write ";***********************************************************************"
local cmdCtr = SCX_CPU1_EVS_CMDPC + 2
...
s get_tbl_to_cvt (ramDir,filterTblName,"A","cpu1_filtertbl211",hostCPU,filterTblPktId)
;; Dump the Destination File Table
s get_tbl_to_cvt (ramDir,fileTblName,"A","cpu1_filetbl211",hostCPU,fileTblPktId)
...
for i = 0 to DS_DEST_FILE_CNT-1 do
  if (SCX_CPU1_DS_DF_TBL[i].FileNameType = DS_BY_COUNT) AND (foundEntry = FALSE) then
    seqFileEntry = i
    foundEntry = TRUE
  endif
enddo
...
for i = 0 to DS_PACKETS_IN_FILTER_TABLE-1 do
  for j = 0 to DS_FILTERS_PER_PACKET-1 do
    if (SCX_CPU1_DS_PF_TBL[i].FilterParams[j].Index = seqFileEntry) AND ;;
       (SCX_CPU1_DS_PF_TBL[i].MessageID <> 0) AND (foundEntry = FALSE) then
      filterEntry = i
      foundEntry = TRUE
    endif
  enddo
...
ut_setupevents "SCX", "CPU1", "TST_DS", TST_DS_MSG1_SENT_INF_EID, "INFO", 1

local expFileWrites = SCX_CPU1_DS_FileWriteCnt + 4
local expPassedPkts = SCX_CPU1_DS_PassedPktCnt + 1

;; Send the command
/SCX_CPU1_TST_DS_SENDMESSAGE MsgID=seqMsgID MsgType=1 Pattern=x'55'
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",TST_DS_MSG1_SENT_INF_EID,"."
endif

;; Sent the Get File Info command to check the File Status
cmdCtr = SCX_CPU1_DS_CMDPC + 1

/SCX_CPU1_DS_GetFileInfo
...
;; is marked "open"
local fileName1 = ""
if (SCX_CPU1_DS_FileState[seqFileEntry].FileName <> "") then
  fileName1 = SCX_CPU1_DS_FileState[seqFileEntry].FileName
  ...
  if (SCX_CPU1_DS_FileState[seqFileEntry].OpenState = DS_OPEN) then
    write "<*> Passed (3001) - The file is marked 'OPEN'"
    ut_setrequirements DS_3001, "P"
  endif

  ;; Check the counters
  if (expFileWrites = SCX_CPU1_DS_FileWriteCnt) AND ;;
     (expPassedPkts = SCX_CPU1_DS_PassedPktCnt) then
    ...
    write "=> File Write Cnt = ",SCX_CPU1_DS_FileWriteCnt,"; Expected ",expFileWrites
    write "=> Passed Pkt Cnt = ",SCX_CPU1_DS_PassedPktCnt,"; Expected ",expPassedPkts
  endif
else
...
write ";***********************************************************************"
;; Setup the event capture
ut_setupevents "SCX", "CPU1", "TST_DS", TST_DS_MSG3_SENT_INF_EID, "INFO", 1

local expHdrUpdates = SCX_CPU1_DS_FileUpdCnt + 1

;; Send the command
/SCX_CPU1_TST_DS_SENDMESSAGE MsgID=seqMsgID MsgType=3 Pattern=x'AA'
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",TST_DS_MSG3_SENT_INF_EID,"."
endif
...
local slashLoc
if (SCX_CPU1_DS_FileState[seqFileEntry].FileName <> "") then
  fileName2 = SCX_CPU1_DS_FileState[seqFileEntry].FileName
  ...
  if (expHdrUpdates = SCX_CPU1_DS_FileUpdCnt) then
    ...
    write "=> Header Update Cnt = ",SCX_CPU1_DS_FileUpdCnt,"; Expected ",expHdrUpdates
  endif
...
write ";***********************************************************************"
;; Setup the event capture
ut_setupevents "SCX", "CPU1", {DSAppName}, DS_CLOSE_CMD_EID, "DEBUG", 1

cmdCtr = SCX_CPU1_DS_CMDPC + 1
;; Send the Close command
/SCX_CPU1_DS_CloseFile FileIndex=seqFileEntry
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed (1004;5002) - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_CLOSE_CMD_EID,"."
  ut_setrequirements DS_1004, "F"
  ut_setrequirements DS_5002, "F"
endif
...
write ";***********************************************************************"
;; Setup the event capture
ut_setupevents "SCX", "CPU1", "TST_DS", TST_DS_MSG5_SENT_INF_EID, "INFO", 1

;; Send the command
/SCX_CPU1_TST_DS_SENDMESSAGE MsgID=seqMsgID MsgType=5 Pattern=x'A5'
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",TST_DS_MSG5_SENT_INF_EID,"."
endif
...
local fileName3 = ""
if (SCX_CPU1_DS_FileState[seqFileEntry].FileName <> "") then
  fileName3 = SCX_CPU1_DS_FileState[seqFileEntry].FileName
...
foundEntry = FALSE
for i = 0 to DS_DEST_FILE_CNT-1 do
  if (SCX_CPU1_DS_DF_TBL[i].FileNameType = DS_BY_TIME) AND (foundEntry = FALSE) then
    timeFileEntry = i
    foundEntry = TRUE
  endif
enddo
...
for i = 0 to DS_PACKETS_IN_FILTER_TABLE-1 do
  for j = 0 to DS_FILTERS_PER_PACKET-1 do
    if (SCX_CPU1_DS_PF_TBL[i].FilterParams[j].Index = timeFileEntry) AND ;;
       (SCX_CPU1_DS_PF_TBL[i].MessageID <> 0) AND (foundEntry = FALSE) then
      filterEntry = i
      foundEntry = TRUE
    endif
...
local timeMsgID = SCX_CPU1_DS_PF_TBL[filterEntry].MessageID
local fileAge = SCX_CPU1_DS_DF_TBL[timeFileEntry].FileAge
...
;; Send the command
/SCX_CPU1_TST_DS_SENDMESSAGE MsgID=timeMsgID MsgType=1 Pattern=x'55'
...
;; is marked "open"
fileName1 = ""
if (SCX_CPU1_DS_FileState[timeFileEntry].FileName <> "") then
  fileName1 = SCX_CPU1_DS_FileState[timeFileEntry].FileName
  ...
  if (SCX_CPU1_DS_FileState[timeFileEntry].OpenState = DS_OPEN) then
    write "<*> Passed (3001) - The file is marked 'OPEN'"
    ut_setrequirements DS_3001, "P"
  endif
else
...
local reportedAge = SCX_CPU1_DS_FileState[timeFileEntry].FileAge
...
while (waitTime > 0) do
  ...
  reportedAge = SCX_CPU1_DS_FileState[timeFileEntry].FileAge
  ...
  if (SCX_CPU1_DS_FileState[timeFileEntry].FileName = "") AND ;;
     (SCX_CPU1_DS_FileState[timeFileEntry].FileAge = 0) then
    waitTime = 0
...
write ";***********************************************************************"
;; Setup the event capture
ut_setupevents "SCX", "CPU1", "TST_DS", TST_DS_MSG4_SENT_INF_EID, "INFO", 1

;; Send the command
/SCX_CPU1_TST_DS_SENDMESSAGE MsgID=seqMsgID MsgType=4 Pattern=x'55'
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",TST_DS_MSG4_SENT_INF_EID,"."
endif
...
write ";***********************************************************************"
;; Setup the event capture
ut_setupevents "SCX", "CPU1", "TST_DS", TST_DS_MSG2_SENT_INF_EID, "INFO", 1

;; Send the command
/SCX_CPU1_TST_DS_SENDMESSAGE MsgID=timeMsgID MsgType=2 Pattern=x'AA'
wait 10
...
write ";***********************************************************************"
;; Setup the event capture
ut_setupevents "SCX", "CPU1", {DSAppName}, DS_BASE_CMD_EID, "DEBUG", 1

cmdCtr = SCX_CPU1_DS_CMDPC + 1
;; Send the command for the Sequence File Type naming entry
/SCX_CPU1_DS_SetBasename FileIndex=seqFileEntry BaseName="sequenceFile"
wait 5
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed (1004;5003) - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_BASE_CMD_EID,"."
  ut_setrequirements DS_1004, "F"
  ut_setrequirements DS_5003, "F"
endif
...
/SCX_CPU1_DS_SetBasename FileIndex=timeFileEntry BaseName="timeFile"
wait 5
...
;; Setup the event capture
ut_setupevents "SCX", "CPU1", "TST_DS", TST_DS_MSG1_SENT_INF_EID, "INFO", 1
ut_setupevents "SCX", "CPU1", {DSAppName}, DS_CREATE_FILE_ERR_EID, "ERROR", 2
ut_setupevents "SCX", "CPU1", "TST_DS", TST_DS_MSG3_SENT_INF_EID, "INFO", 3

local expectedErrCnt = SCX_CPU1_DS_FileWriteErrCnt + 2
...
;; Send the command
/SCX_CPU1_TST_DS_SENDMESSAGE MsgID=timeMsgID MsgType=3 Pattern=x'55'
wait 5
...
if (SCX_CPU1_find_event[2].num_found_messages = 2) then
...
if (expectedErrCnt = SCX_CPU1_DS_FileWriteErrCnt) then
...
if (SCX_CPU1_DS_FileState[seqFileEntry].OpenState = DS_CLOSED) then
...
if (SCX_CPU1_DS_FileState[timeFileEntry].OpenState = DS_CLOSED) then
...
if (SCX_CPU1_DS_FileState[seqFileEntry].EnableState = DS_DISABLED) then
...
if (SCX_CPU1_DS_FileState[timeFileEntry].EnableState = DS_DISABLED) then
...
ut_setupevents "SCX","CPU1","CFE_TBL",CFE_TBL_VAL_REQ_MADE_INF_EID,"DEBUG", 1
ut_setupevents "SCX","CPU1","CFE_TBL",CFE_TBL_VALIDATION_INF_EID,"INFO", 2
...
if (UT_TW_Status = UT_Success) then
  ...
  if (SCX_CPU1_find_event[1].num_found_messages = 1) then
    write "<*> Passed - Event Msg ",SCX_CPU1_find_event[1].eventid," Found!"
  else
...
ut_setupevents "SCX","CPU1","CFE_TBL",CFE_TBL_LOAD_PEND_REQ_INF_EID,"DEBUG",1
ut_setupevents "SCX","CPU1","CFE_TBL",CFE_TBL_UPDATE_SUCCESS_INF_EID,"INFO",2
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) then
  write "<*> Passed - Event message ",SCX_CPU1_find_event[1].eventid, " received"
else
  write "<!> Failed - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",CFE_TBL_LOAD_PEND_REQ_INF_EID, "."
endif
...
if (SCX_CPU1_find_event[2].num_found_messages = 1) then
  ...
  Write "<*> Passed - Event Msg ",SCX_CPU1_find_event[2].eventid," Found!"
else
...
local cmdCtr = SCX_CPU1_DS_CMDPC + 1
;; Send the Close command
/SCX_CPU1_DS_CloseFile FileIndex=seqFileEntry
...
write ";*********************************************************************"
;; setup event capture
ut_setupevents "SCX", "CPU1", {DSAppName}, DS_CLOSE_CMD_ERR_EID, "ERROR", 1
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1005) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",DS_CLOSE_CMD_ERR_EID, "."
  ut_setrequirements DS_1005, "F"
endif
...
;; Send the Close command
/SCX_CPU1_DS_CloseFile FileIndex=DS_DEST_FILE_CNT
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed (1005) - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_CLOSE_CMD_ERR_EID,"."
  ut_setrequirements DS_1005, "F"
endif
...
cmdCtr = SCX_CPU1_DS_CMDPC + 1
;; Send the command for the Sequence File Type naming entry
/SCX_CPU1_DS_SetBasename FileIndex=seqFileEntry BaseName="newSeq"
wait 5
...
write ";*********************************************************************"
;; Dump the Destination File Table
s get_tbl_to_cvt (ramDir,fileTblName,"A","cpu1_filetbl35",hostCPU,fileTblPktId)

;; Verify the table contains the change made
if (SCX_CPU1_DS_DF_TBL[seqFileEntry].Basename = "newSeq") then
...
cmdCtr = SCX_CPU1_DS_CMDPC + 1
;; Send the command for the Sequence File Type naming entry
/SCX_CPU1_DS_SetBasename FileIndex=seqFileEntry BaseName=""
wait 5
...
write ";*********************************************************************"
;; Dump the Destination File Table
s get_tbl_to_cvt (ramDir,fileTblName,"A","cpu1_filetbl38",hostCPU,fileTblPktId)

;; Verify the table contains the change made
if (SCX_CPU1_DS_DF_TBL[seqFileEntry].Basename = "") then
  write "<*> Passed (5003) - Table entry reflects the new Basename."
  ut_setrequirements DS_5003, "P"
else
...
write ";***********************************************************************"
;; setup event capture
ut_setupevents "SCX", "CPU1", {DSAppName}, DS_BASE_CMD_ERR_EID, "ERROR", 1
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1005) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",DS_BASE_CMD_ERR_EID, "."
  ut_setrequirements DS_1005, "F"
endif
...
errcnt = SCX_CPU1_DS_CMDEC + 1
;; Send the command for the Sequence File Type naming entry
/SCX_CPU1_DS_SetBasename FileIndex=DS_DEST_FILE_CNT BaseName="badSeq"
wait 5
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed (1005) - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_BASE_CMD_ERR_EID,"."
  ut_setrequirements DS_1005, "F"
endif
...
write ";***********************************************************************"
;; Setup the event capture
ut_setupevents "SCX", "CPU1", {DSAppName}, DS_SEQ_CMD_EID, "DEBUG", 1

cmdCtr = SCX_CPU1_DS_CMDPC + 1
;; Send the command
/SCX_CPU1_DS_SetFileSeqCtr FileIndex=seqFileEntry SeqCount=99
wait 5
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed (1004;5004) - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_SEQ_CMD_EID,"."
  ut_setrequirements DS_1004, "F"
  ut_setrequirements DS_5004, "F"
endif
...
write ";***********************************************************************"
;; Dump the Destination File Table
s get_tbl_to_cvt (ramDir,fileTblName,"A","cpu1_filetbl313",hostCPU,fileTblPktId)

;; Verify the table contains the change made
if (SCX_CPU1_DS_DF_TBL[seqFileEntry].SeqCnt = 99) then
...
write ";***********************************************************************"
;; setup event capture
ut_setupevents "SCX", "CPU1", {DSAppName}, DS_SEQ_CMD_ERR_EID, "ERROR", 1
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1005) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",DS_SEQ_CMD_ERR_EID, "."
  ut_setrequirements DS_1005, "F"
endif
...
;; Send the command
/SCX_CPU1_DS_SetFileSeqCtr FileIndex=DS_DEST_FILE_CNT SeqCount=155
wait 5
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed (1005) - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_SEQ_CMD_ERR_EID,"."
  ut_setrequirements DS_1005, "F"
endif
...
write ";***********************************************************************"
;; Setup the event capture
ut_setupevents "SCX", "CPU1", {DSAppName}, DS_STATE_CMD_EID, "DEBUG", 1

cmdCtr = SCX_CPU1_DS_CMDPC + 1
;; Send the command
/SCX_CPU1_DS_DisableFile FileIndex=seqFileEntry
wait 5
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed (1004;5006) - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_STATE_CMD_EID,"."
  ut_setrequirements DS_1004, "F"
  ut_setrequirements DS_5006, "F"
endif
...
write ";***********************************************************************"
;; Dump the Destination File Table
s get_tbl_to_cvt (ramDir,fileTblName,"A","cpu1_filetbl318",hostCPU,fileTblPktId)

;; Verify the table contains the change made
if (SCX_CPU1_DS_DF_TBL[seqFileEntry].FileState = DS_DISABLED) then
...
local pktCtr = SCX_CPU1_DS_FilteredPktCnt + 1
...
if (SCX_CPU1_DS_FilteredPktCnt = pktCtr) then
  write "<*> Passed (5006) - The filtered Packet Counter incremented indicating that the message was not stored."
  ut_setrequirements DS_5006, "P"
else
  write "<!> Failed (5006) - The filtered Pkt counter did not increment as expected. Expected ",pktCtr, " rcv'd ",SCX_CPU1_DS_FilteredPktCnt
  ut_setrequirements DS_5006, "F"
endif
...
write ";***********************************************************************"
;; setup event capture
ut_setupevents "SCX", "CPU1", {DSAppName}, DS_STATE_CMD_ERR_EID, "ERROR", 1
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1005) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",DS_STATE_CMD_ERR_EID, "."
  ut_setrequirements DS_1005, "F"
endif
...
;; Send the command
/SCX_CPU1_DS_DisableFile FileIndex=DS_DEST_FILE_CNT
wait 5
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed (1005) - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_STATE_CMD_ERR_EID,"."
  ut_setrequirements DS_1005, "F"
endif
...
cmdCtr = SCX_CPU1_DS_CMDPC + 1
;; Send the command
/SCX_CPU1_DS_EnableFile FileIndex=seqFileEntry
wait 5
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed (1004;5005) - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_STATE_CMD_EID,"."
  ut_setrequirements DS_1004, "F"
  ut_setrequirements DS_5005, "F"
endif
...
;; Check that the Enable State in HK indicates disabled
if (SCX_CPU1_DS_FileState[seqFileEntry].EnableState = DS_ENABLED) then
...
write ";***********************************************************************"
;; Dump the Destination File Table
s get_tbl_to_cvt (ramDir,fileTblName,"A","cpu1_filetbl323",hostCPU,fileTblPktId)

;; Verify the table contains the change made
if (SCX_CPU1_DS_DF_TBL[seqFileEntry].FileState = DS_ENABLED) then
...
local fileSize = SCX_CPU1_DS_FileState[seqFileEntry].FileSize
pktCtr = SCX_CPU1_DS_PassedPktCnt + 1
local writeCnt = SCX_CPU1_DS_FileWriteCnt + 1

;; Setup the event capture
ut_setupevents "SCX", "CPU1", "TST_DS", TST_DS_MSG1_SENT_INF_EID, "INFO", 1

;; Send the command
/SCX_CPU1_TST_DS_SENDMESSAGE MsgID=seqMsgID MsgType=1 Pattern=x'AA'
...
if (SCX_CPU1_DS_FileState[seqFileEntry].FileSize > fileSize) AND ;;
   (SCX_CPU1_DS_PassedPktCnt = pktCtr) AND ;;
   (SCX_CPU1_DS_FileWriteCnt = writeCnt) then
...
;; Send the command
/SCX_CPU1_DS_EnableFile FileIndex=DS_DEST_FILE_CNT
wait 5
...
write ";***********************************************************************"
;; Setup the event capture
ut_setupevents "SCX", "CPU1", {DSAppName}, DS_NTYPE_CMD_EID, "DEBUG", 1

cmdCtr = SCX_CPU1_DS_CMDPC + 1
;; Send the command
/SCX_CPU1_DS_SetFileType FileIndex=seqFileEntry FileType=DS_BY_TIME
wait 5
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed (1004;5011) - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_NTYPE_CMD_EID,"."
  ut_setrequirements DS_1004, "F"
  ut_setrequirements DS_5011, "F"
endif
...
write ";***********************************************************************"
;; Dump the Destination File Table
s get_tbl_to_cvt (ramDir,fileTblName,"A","cpu1_filetbl328",hostCPU,fileTblPktId)

;; Verify the table contains the change made
if (p@SCX_CPU1_DS_DF_TBL[seqFileEntry].FileNameType = "Time") then
...
;; Send the command
/SCX_CPU1_TST_DS_SENDMESSAGE MsgID=seqMsgID MsgType=3 Pattern=x'BB'
...
write ";***********************************************************************"
;; setup event capture
ut_setupevents "SCX", "CPU1", {DSAppName}, DS_NTYPE_CMD_ERR_EID, "ERROR", 1
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1005) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",DS_NTYPE_CMD_ERR_EID, "."
  ut_setrequirements DS_1005, "F"
endif
...
;; Send the command
/SCX_CPU1_DS_SetFileType FileIndex=DS_DEST_FILE_CNT FileType=DS_BY_TIME
wait 5
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed (1005) - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_NTYPE_CMD_ERR_EID,"."
  ut_setrequirements DS_1005, "F"
endif
...
;; Send the command
/SCX_CPU1_DS_SetFileType FileIndex=seqFileEntry FileType=3
wait 5
...
write ";***********************************************************************"
;; Setup the event capture
ut_setupevents "SCX", "CPU1", {DSAppName}, DS_PATH_CMD_EID, "DEBUG", 1

cmdCtr = SCX_CPU1_DS_CMDPC + 1
;; Send the command
/SCX_CPU1_DS_SetPath FileIndex=seqFileEntry PathName="/cf/"
wait 5
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed (1004;5012) - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_PATH_CMD_EID,"."
  ut_setrequirements DS_1004, "F"
  ut_setrequirements DS_5012, "F"
endif
...
write ";***********************************************************************"
;; Dump the Destination File Table
s get_tbl_to_cvt (ramDir,fileTblName,"A","cpu1_filetbl333",hostCPU,fileTblPktId)

;; Verify the table contains the change made
if (p@SCX_CPU1_DS_DF_TBL[seqFileEntry].Pathname = "/cf/") then
...
;; Send the command
/SCX_CPU1_TST_DS_SENDMESSAGE MsgID=seqMsgID MsgType=3 Pattern=x'A5'
...
write ";***********************************************************************"
;; setup event capture
ut_setupevents "SCX", "CPU1", {DSAppName}, DS_PATH_CMD_ERR_EID, "ERROR", 1
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1005) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",DS_PATH_CMD_ERR_EID, "."
  ut_setrequirements DS_1005, "F"
endif
...
;; Send the command
/SCX_CPU1_DS_SetPath FileIndex=DS_DEST_FILE_CNT PathName="/ram/"
wait 5
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed (1005) - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_PATH_CMD_ERR_EID,"."
  ut_setrequirements DS_1005, "F"
endif
...
;; Send the command
/SCX_CPU1_DS_SetPath FileIndex=seqFileEntry PathName=""
wait 5
...
;; Send the command
/SCX_CPU1_DS_SetPath FileIndex=seqFileEntry PathName="/ram/waltsDir"
wait 5
...
;; Send the command
/SCX_CPU1_TST_DS_SENDMESSAGE MsgID=seqMsgID MsgType=1 Pattern=x'BB'
...
if (SCX_CPU1_find_event[2].num_found_messages = 1) then
  ...
  write "<!> Failed - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_CREATE_FILE_ERR_EID,"."
endif
...
;; Send the command
/SCX_CPU1_DS_SetExtension FileIndex=timeFileEntry Extension=".dat"
wait 5
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed (1004;5013) - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_EXT_CMD_EID,"."
  ut_setrequirements DS_1004, "F"
  ut_setrequirements DS_5013, "F"
endif
...
write ";***********************************************************************"
;; Dump the Destination File Table
s get_tbl_to_cvt (ramDir,fileTblName,"A","cpu1_filetbl338",hostCPU,fileTblPktId)

;; Verify the table contains the change made
if (p@SCX_CPU1_DS_DF_TBL[timeFileEntry].Extension = ".dat") then
...
;; Send the command
/SCX_CPU1_TST_DS_SENDMESSAGE MsgID=timeMsgID MsgType=3 Pattern=x'AA'
...
write ";***********************************************************************"
;; setup event capture
ut_setupevents "SCX", "CPU1", {DSAppName}, DS_EXT_CMD_ERR_EID, "ERROR", 1
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1005) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",DS_EXT_CMD_ERR_EID, "."
  ut_setrequirements DS_1005, "F"
endif
...
;; Send the command
/SCX_CPU1_DS_SetExtension FileIndex=DS_DEST_FILE_CNT Extension=".icu"
wait 5
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed (1005) - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_EXT_CMD_ERR_EID,"."
  ut_setrequirements DS_1005, "F"
endif
...
;; Send the command
/SCX_CPU1_DS_SetExtension FileIndex=timeFileEntry Extension=""
wait 5
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed (1005) - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_EXT_CMD_EID,"."
  ut_setrequirements DS_1005, "F"
endif
...
/SCX_CPU1_DS_SetPath FileIndex=seqFileEntry PathName="/ram/"
wait 5

;; Send the command set the file type back to sequence file naming
/SCX_CPU1_DS_SetFileType FileIndex=seqFileEntry FileType=DS_BY_COUNT
wait 5
...
ut_setupevents "SCX", "CPU1", {DSAppName}, DS_SIZE_CMD_EID, "DEBUG", 1

cmdCtr = SCX_CPU1_DS_CMDPC + 1
;; Send the command
/SCX_CPU1_DS_SetMaxFileSize FileIndex=seqFileEntry MaxSize=5500
wait 5
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed (1004;5014) - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_SIZE_CMD_EID,"."
  ut_setrequirements DS_1004, "F"
  ut_setrequirements DS_5014, "F"
endif
...
write ";***********************************************************************"
;; Dump the Destination File Table
s get_tbl_to_cvt (ramDir,fileTblName,"A","cpu1_filetbl343",hostCPU,fileTblPktId)

;; Verify the table contains the change made
if (p@SCX_CPU1_DS_DF_TBL[seqFileEntry].FileSize = 5500) then
...
expFileWrites = SCX_CPU1_DS_FileWriteCnt + 1
expPassedPkts = SCX_CPU1_DS_PassedPktCnt + 1

/SCX_CPU1_TST_DS_SENDMESSAGE MsgID=seqMsgID MsgType=3 Pattern=x'55'
wait 5
...
/SCX_CPU1_TST_DS_SENDMESSAGE MsgID=seqMsgID MsgType=3 Pattern=x'FF'
wait 5
...
/SCX_CPU1_TST_DS_SENDMESSAGE MsgID=seqMsgID MsgType=3 Pattern=x'5A'
wait 5
...
write ";***********************************************************************"
;; setup event capture
ut_setupevents "SCX", "CPU1", {DSAppName}, DS_SIZE_CMD_ERR_EID, "ERROR", 1
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1005) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",DS_SIZE_CMD_ERR_EID, "."
  ut_setrequirements DS_1005, "F"
endif
...
ut_setupevents "SCX", "CPU1", {DSAppName}, DS_AGE_CMD_EID, "DEBUG", 1

cmdCtr = SCX_CPU1_DS_CMDPC + 1
;; Send the command
/SCX_CPU1_DS_SetMaxFileAge FileIndex=timeFileEntry MaxAge=90
wait 5
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed (1004;5015) - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_AGE_CMD_EID,"."
  ut_setrequirements DS_1004, "F"
  ut_setrequirements DS_5015, "F"
endif
...
write ";***********************************************************************"
;; Dump the Destination File Table
s get_tbl_to_cvt (ramDir,fileTblName,"A","cpu1_filetbl347",hostCPU,fileTblPktId)

;; Verify the table contains the change made
if (p@SCX_CPU1_DS_DF_TBL[timeFileEntry].FileAge = 90) then
...
;; is marked "open"
if (SCX_CPU1_DS_FileState[timeFileEntry].FileName <> "") then
  fileName2 = SCX_CPU1_DS_FileState[timeFileEntry].FileName
  write "<*> Passed (3000) - A file was created with name '",fileName2,"'"
  ut_setrequirements DS_3000, "P"
...
;; Loop here for each HK packet and see the Age updating
;; This verifies DS3002
fileAge = SCX_CPU1_DS_DF_TBL[timeFileEntry].FileAge
reportedAge = SCX_CPU1_DS_FileState[timeFileEntry].FileAge
waitTime = fileAge - reportedAge
...
write ";***********************************************************************"
;; setup event capture
ut_setupevents "SCX", "CPU1", {DSAppName}, DS_AGE_CMD_ERR_EID, "ERROR", 1
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1005) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",DS_AGE_CMD_ERR_EID, "."
  ut_setrequirements DS_1005, "F"
endif
...
ut_setupevents "SCX", "CPU1", {DSAppName}, DS_ENADIS_CMD_EID, "DEBUG", 1
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed (1004;5000) - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_ENADIS_CMD_EID,"."
  ut_setrequirements DS_1004, "F"
  ut_setrequirements DS_5000, "F"
endif

;; Verify that the state in HK indicates 'DISABLED'
if (p@SCX_CPU1_DS_AppEnaState = "Disabled") then
...
;; Setup the event capture
ut_setupevents "SCX", "CPU1", "TST_DS", TST_DS_MSG3_SENT_INF_EID, "INFO", 1
ut_setupevents "SCX", "CPU1", "TST_DS", TST_DS_MSG1_SENT_INF_EID, "INFO", 2
ut_setupevents "SCX", "CPU1", "TST_DS", TST_DS_MSG2_SENT_INF_EID, "INFO", 3

pktCtr = SCX_CPU1_DS_DisabledPktCnt + 3
fileSize = SCX_CPU1_DS_FileState[seqFileEntry].FileSize
...
/SCX_CPU1_TST_DS_SENDMESSAGE MsgID=seqMsgID MsgType=1 Pattern=x'55'
wait 5
/SCX_CPU1_TST_DS_SENDMESSAGE MsgID=seqMsgID MsgType=2 Pattern=x'BB'
wait 5

;; Verify the disabled packet counter incremented and 
;; the file status did not change
if (SCX_CPU1_DS_DisabledPktCnt = pktCtr) AND ;;
   (SCX_CPU1_DS_FileState[seqFileEntry].FileSize = fileSize) then
  ...
  write "Expected file size of ",fileSize,"; Got ",SCX_CPU1_DS_FileState[seqFileEntry].FileSize
  write "Expected Disabled Ctr ",pktCtr,"; Got ",SCX_CPU1_DS_DisabledPktCnt
  ut_setrequirements DS_8000, "F"
endif

;; Verify that the events were generated
if (SCX_CPU1_find_event[1].num_found_messages = 1) AND ;;
   (SCX_CPU1_find_event[2].num_found_messages = 1) AND ;;
   (SCX_CPU1_find_event[3].num_found_messages = 1) then
...
write ";***********************************************************************"
;; setup event capture
ut_setupevents "SCX", "CPU1", {DSAppName}, DS_ENADIS_CMD_ERR_EID, "ERROR", 1
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1005) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",DS_ENADIS_CMD_ERR_EID, "."
  ut_setrequirements DS_1005, "F"
endif
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed (1004;5001) - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_ENADIS_CMD_EID,"."
  ut_setrequirements DS_1004, "F"
  ut_setrequirements DS_5001, "F"
endif

;; Verify that the state in HK indicates 'ENABLED'
if (p@SCX_CPU1_DS_AppEnaState = "Enabled") then
...
pktCtr = SCX_CPU1_DS_PassedPktCnt + 3
fileSize = SCX_CPU1_DS_FileState[seqFileEntry].FileSize
...
;; Verify the disabled packet counter incremented and 
;; the file status did not change
if (SCX_CPU1_DS_PassedPktCnt = pktCtr) AND ;;
   (SCX_CPU1_DS_FileState[seqFileEntry].FileSize > fileSize) then
...
;; Verify that the events were generated
if (SCX_CPU1_find_event[1].num_found_messages = 2) then
  write "<*> Passed - Expected Event Msgs were generated."
else
  write "<!> Failed - Expected Event messages were not rcv'd from TST_DS."
endif
...
write ";***********************************************************************"
;; Setup the event capture
ut_setupevents "SCX", "CPU1", {DSAppName}, DS_CLOSE_ALL_CMD_EID, "DEBUG", 1

cmdCtr = SCX_CPU1_DS_CMDPC + 1

;; Send the command to close all file
/SCX_CPU1_DS_CloseAllFiles
...
write ";***********************************************************************"
;; setup event capture
ut_setupevents "SCX", "CPU1", {DSAppName}, DS_CLOSE_ALL_CMD_ERR_EID, "ERROR", 1
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1005) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",DS_CLOSE_ALL_CMD_ERR_EID, "."
  ut_setrequirements DS_1005, "F"
endif
...
write ";*********************************************************************"
write ";  End procedure SCX_CPU1_ds_filewrite"
write ";*********************************************************************"
ENDPROC
