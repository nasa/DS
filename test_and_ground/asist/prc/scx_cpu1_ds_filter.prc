...
s get_tbl_to_cvt (ramDir,filterTblName,"A","cpu1_filtertbl16",hostCPU,filterTblPktId)

local entryCount = 0
;; Verify the table contents loaded in Step 1.2
for i = 0 to DS_PACKETS_IN_FILTER_TABLE-1 do
  if (SCX_CPU1_DS_PF_TBL[i].MessageID <> DS_UNUSED) then
    entryCount = entryCount + 1
    write "==> MessageID = ",%hex(SCX_CPU1_DS_PF_TBL[i].MessageID,4)
    for j = 0 to DS_FILTERS_PER_PACKET-1 do
      if (SCX_CPU1_DS_PF_TBL[i].FilterParams[j].FilterType <> DS_UNUSED) then
	write "==> File Index = ",SCX_CPU1_DS_PF_TBL[i].FilterParams[j].Index
	write "==> Filter Type = ",SCX_CPU1_DS_PF_TBL[i].FilterParams[j].FilterType
	write "==> N Value = ",SCX_CPU1_DS_PF_TBL[i].FilterParams[j].N_Value
	write "==> X Value = ",SCX_CPU1_DS_PF_TBL[i].FilterParams[j].X_Value
	write "==> O Value = ",SCX_CPU1_DS_PF_TBL[i].FilterParams[j].O_Value
      endif
    enddo
  else
...
s get_tbl_to_cvt (ramDir,fileTblName,"A","cpu1_filetbl17",hostCPU,fileTblPktId)

entryCount = 0
;; Verify the table contents loaded in Step 1.2
for i = 0 to DS_DEST_FILE_CNT-1 do
  if (SCX_CPU1_DS_DF_TBL[i].FileNameType <> DS_UNUSED) then
    entryCount = entryCount + 1
    write "==> Pathname = '",SCX_CPU1_DS_DF_TBL[i].Pathname,"'"
    write "==> Basename = '",SCX_CPU1_DS_DF_TBL[i].Basename,"'"
    write "==> Extension = '",SCX_CPU1_DS_DF_TBL[i].Extension,"'"
    write "==> Name Type = ",p@SCX_CPU1_DS_DF_TBL[i].FileNameType
    write "==> State     = ",p@SCX_CPU1_DS_DF_TBL[i].FileState
    write "==> Max Size = ",SCX_CPU1_DS_DF_TBL[i].FileSize
    write "==> Max Age = ",SCX_CPU1_DS_DF_TBL[i].FileAge
    write "==> Seq Count = ",SCX_CPU1_DS_DF_TBL[i].SeqCnt
  else
    i = DS_DEST_FILE_CNT 
  endif
enddo
...
for i = 0 to DS_PACKETS_IN_FILTER_TABLE-1 do
  if (SCX_CPU1_DS_PF_TBL[i].MessageID <> 0 AND foundEntry = FALSE) then
    ;; Check for max files being used
    entryCount = 0
    for j = 0 to DS_FILTERS_PER_PACKET-1 do
      if (SCX_CPU1_DS_PF_TBL[i].FilterParams[j].FilterType = DS_BY_COUNT) then
	entryCount = entryCount + 1
        seqFileIDs[j] = SCX_CPU1_DS_PF_TBL[i].FilterParams[j].Index
      endif
    enddo
...
;; Send the command
/SCX_CPU1_TST_DS_SENDMESSAGE MsgID=seqMsgID MsgType=1 Pattern=x'5A'
...
if (expPassedPkts = SCX_CPU1_DS_PassedPktCnt) then
  write "<*> Passed (2000) - Passed packet counter incremented properly."
  ut_setrequirements DS_2000, "P"
else
  write "<!> Failed (2000) - Passed packet counter not set properly. Expected '",expPassedPkts,"'. Count = '",SCX_CPU1_DS_PassedPktCnt,"'."
...
for i = 0 to DS_FILTERS_PER_PACKET-1 do
  hkSlot = seqFileIDs[i]
  if (SCX_CPU1_DS_FileState[hkSlot].FileName <> "") then
    fileName = SCX_CPU1_DS_FileState[hkSlot].FileName
...
for i = 0 to DS_PACKETS_IN_FILTER_TABLE-1 do
  if (SCX_CPU1_DS_PF_TBL[i].MessageID <> 0 AND foundEntry = FALSE) then
    ...
    entryCount = 0
    for j = 0 to DS_FILTERS_PER_PACKET-1 do
      if (SCX_CPU1_DS_PF_TBL[i].FilterParams[j].FilterType = DS_BY_TIME) then
	entryCount = entryCount + 1
        timeFileIDs[j] = SCX_CPU1_DS_PF_TBL[i].FilterParams[j].Index
      endif
    enddo
    if (entryCount = DS_FILTERS_PER_PACKET) then
      filterEntry = i
...
;; Send the command
/SCX_CPU1_TST_DS_SENDMESSAGE MsgID=timeMsgID MsgType=1 Pattern=x'BB'
...
for i = 0 to DS_FILTERS_PER_PACKET-1 do
  ...
  seqFileName = SCX_CPU1_DS_FileState[seqFileIDs[i]].FileName
  timeFileName = SCX_CPU1_DS_FileState[timeFileIDs[i]].FileName

  ;; Setup the event capture
  ut_setupevents "SCX", "CPU1", {DSAppName}, DS_CLOSE_CMD_EID, "DEBUG", 1

  cmdCtr = SCX_CPU1_DS_CMDPC + 2
  ;; Send the Close commands
  /SCX_CPU1_DS_CloseFile FileIndex=seqFileIDs[i]
  /SCX_CPU1_DS_CloseFile FileIndex=timeFileIDs[i]
  ...
  ;; Check for the event message
  ut_tlmwait SCX_CPU1_find_event[1].num_found_messages, 2
  if (UT_TW_Status = UT_Success) then
...
foundEntry = FALSE
for i = 0 to DS_PACKETS_IN_FILTER_TABLE-1 do
  if (SCX_CPU1_DS_PF_TBL[i].MessageID <> seqMsgID) AND ;;
     (SCX_CPU1_DS_PF_TBL[i].MessageID <> timeMsgID AND foundEntry = FALSE) then
    filterEntry = i
    foundEntry = TRUE
  endif
enddo
...
local oneMsgID = SCX_CPU1_DS_PF_TBL[filterEntry].MessageID
local msgsToSend = SCX_CPU1_DS_PF_TBL[filterEntry].FilterParams[0].X_Value
local msgsStored = SCX_CPU1_DS_PF_TBL[filterEntry].FilterParams[0].N_Value
local storageOffset = SCX_CPU1_DS_PF_TBL[filterEntry].FilterParams[0].O_Value
...
expPassedPkts = SCX_CPU1_DS_PassedPktCnt + msgsStored
local expFilteredPkts = SCX_CPU1_DS_FilteredPktCnt + (msgsToSend - msgsStored)
...
write "==> Passed Packet Cnt = ",SCX_CPU1_DS_PassedPktCnt," expected Cnt = ",expPassedPkts
write "==> File Write Cnt = ",SCX_CPU1_DS_FileWriteCnt," expected Cnt = ",expFileWrites
...
for i = 1 to msgsToSend do
  ...
  dataPattern = i
  ;; Send the command
  /SCX_CPU1_TST_DS_SENDMESSAGE MsgID=oneMsgID MsgType=1 Pattern=dataPattern
  wait 5
enddo

;; Check for the event messages
if (SCX_CPU1_find_event[1].num_found_messages = msgsToSend) then
  write "<*> Passed - Rcv'd the expected number of Send events."
else
  write "<!> Failed - Expected ", msgsToSend, " Send events. Rcv'd ",SCX_CPU1_find_event[1].num_found_messages
endif

;; Should have 'N' messages stored at this point
;; Verify that the Passed Packet counter incremented properly
if (expPassedPkts = SCX_CPU1_DS_PassedPktCnt) AND ;;
   (expFilteredPkts = SCX_CPU1_DS_FilteredPktCnt) AND ;;
   (expFileWrites = SCX_CPU1_DS_FileWriteCnt) then
  write "<*> Passed (2000) - Counters incremented properly."
  ut_setrequirements DS_2000, "P"
else
  write "<!> Failed (2000) - Counters not set properly. Expected Passed pkt = '",expPassedPkts,"' HK = '",SCX_CPU1_DS_PassedPktCnt,"'. Expected File Writes = '",expFileWrites,"' HK  = '",SCX_CPU1_DS_FileWriteCnt,"'. Expected Filtered Pkt = ",expFilteredPkts,"' HK = '",SCX_CPU1_DS_FilteredPktCnt
  ut_setrequirements DS_2000, "F"
endif
...
local fileTblEntry = SCX_CPU1_DS_PF_TBL[filterEntry].FilterParams[0].Index
if (SCX_CPU1_DS_FileState[fileTblEntry].FileName <> "") then
  fileName = SCX_CPU1_DS_FileState[fileTblEntry].FileName
...
cmdCtr = SCX_CPU1_DS_CMDPC + 1
;; Send the Close commands
/SCX_CPU1_DS_CloseFile FileIndex=fileTblEntry
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {DSAppName}, DS_PARMS_CMD_EID, "DEBUG", 1
...
/SCX_CPU1_DS_SetFilterParams MessageID=oneMsgID ParamIndex=0 N_Value=msgsStored X_Value=msgsToSend O_Value=storageOffset
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed (1004;5010) - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_PARMS_CMD_EID,"."
  ut_setrequirements DS_1004, "F"
  ut_setrequirements DS_5010, "F"
endif
...
write ";***********************************************************************"
;; Dump the Packet Filter Table
s get_tbl_to_cvt (ramDir,filterTblName,"A","cpu1_filtertbl27",hostCPU,filterTblPktId)

;; Verify the table contents reflect the commanded changes
if (SCX_CPU1_DS_PF_TBL[filterEntry].FilterParams[0].N_Value = msgsStored) AND ;;
   (SCX_CPU1_DS_PF_TBL[filterEntry].FilterParams[0].X_Value = msgsToSend) AND ;;
   (SCX_CPU1_DS_PF_TBL[filterEntry].FilterParams[0].O_Value = storageOffset) then
...
;; Setup the message counters to expect
expPassedPkts = SCX_CPU1_DS_PassedPktCnt + msgsStored
expFilteredPkts = SCX_CPU1_DS_FilteredPktCnt + (msgsToSend - msgsStored)
...
;; Verify the file was created
fileTblEntry = SCX_CPU1_DS_PF_TBL[filterEntry].FilterParams[0].Index
if (SCX_CPU1_DS_FileState[fileTblEntry].FileName <> "") then
  fileName = SCX_CPU1_DS_FileState[fileTblEntry].FileName
  write "<*> Passed (2002.1;3000) - A file was created with name '",fileName,"'"
  ut_setrequirements DS_20021, "P"
...
for i = 0 to DS_PACKETS_IN_FILTER_TABLE-1 do
  if (SCX_CPU1_DS_PF_TBL[i].MessageID <> seqMsgID) AND ;;
     (SCX_CPU1_DS_PF_TBL[i].MessageID <> timeMsgID) AND ;;
     (SCX_CPU1_DS_PF_TBL[i].MessageID <> oneMsgID AND foundEntry = FALSE) then
    xFilterEntry = i
    foundEntry = TRUE
  endif
enddo
...
local xMsgID = SCX_CPU1_DS_PF_TBL[xFilterEntry].MessageID
local nValue = SCX_CPU1_DS_PF_TBL[xFilterEntry].FilterParams[0].N_Value
local oValue = SCX_CPU1_DS_PF_TBL[xFilterEntry].FilterParams[0].O_Value
local xFileIndex = SCX_CPU1_DS_PF_TBL[xFilterEntry].FilterParams[0].Index
...
/SCX_CPU1_DS_SetFilterParams MessageID=oneMsgID ParamIndex=0 N_Value=0 X_Value=msgsToSend O_Value=storageOffset
...
cmdCtr = SCX_CPU1_DS_CMDPC + 1
;; Send the Set Fiter Params Command
/SCX_CPU1_DS_SetFilterParams MessageID=xMsgID ParamIndex=0 N_Value=0 X_Value=0 O_Value=oValue
...
;; Verify the table contents reflect the commanded changes
if (SCX_CPU1_DS_PF_TBL[filterEntry].FilterParams[0].N_Value = 0) AND ;;
   (SCX_CPU1_DS_PF_TBL[filterEntry].FilterParams[0].X_Value = msgsToSend) AND ;;
   (SCX_CPU1_DS_PF_TBL[filterEntry].FilterParams[0].O_Value = storageOffset) AND ;;
   (SCX_CPU1_DS_PF_TBL[xFilterEntry].FilterParams[0].N_Value = 0) AND ;;
   (SCX_CPU1_DS_PF_TBL[xFilterEntry].FilterParams[0].X_Value = 0) AND ;;
   (SCX_CPU1_DS_PF_TBL[xFilterEntry].FilterParams[0].O_Value = oValue) then
  write "<*> Passed (5010) - Parameter changes are reflected in the table."
  ut_setrequirements DS_5010, "P"
else  
  write "<!> Failed (5010) - Expected parameter changes were not set in the Table entry."
...
for i = 1 to msgsToSend do
  ;; Send the command
  /SCX_CPU1_TST_DS_SENDMESSAGE MsgID=oneMsgID MsgType=1 Pattern=x'55'
  wait 5
enddo

/SCX_CPU1_TST_DS_SENDMESSAGE MsgID=xMsgID MsgType=1 Pattern=x'AA'
wait 5
...
;; Check for the event messages
if (SCX_CPU1_find_event[1].num_found_messages = msgsToSend+1) then
  write "<*> Passed - Rcv'd the expected number of Send events."
else
  write "<!> Failed - Expected ",msgsToSend+1," Send events. Rcv'd ",SCX_CPU1_find_event[1].num_found_messages
endif
...
fileName = SCX_CPU1_DS_FileState[xFileIndex].FileName
if (fileName = "") then
  write "<*> Passed (2001.1) - A file was not created."
  ut_setrequirements DS_20011, "P"
else
...
for i = 0 to DS_PACKETS_IN_FILTER_TABLE-1 do
  if (SCX_CPU1_DS_PF_TBL[i].MessageID <> seqMsgID) AND ;;
     (SCX_CPU1_DS_PF_TBL[i].MessageID <> timeMsgID) AND ;;
     (SCX_CPU1_DS_PF_TBL[i].MessageID <> oneMsgID) AND ;;
     (SCX_CPU1_DS_PF_TBL[i].FilterParams[0].FilterType = DS_BY_COUNT) AND ;;
     (SCX_CPU1_DS_PF_TBL[i].MessageID <> xMsgID AND foundEntry = FALSE) then
    filterEntry = i
    foundEntry = TRUE
  endif
enddo
...
local timeFileMsgID = SCX_CPU1_DS_PF_TBL[filterEntry].MessageID
...
write ";***********************************************************************"
ut_setupevents "SCX", "CPU1", {DSAppName}, DS_FILE_CMD_EID, "DEBUG", 1

cmdCtr = SCX_CPU1_DS_CMDPC + 1
;; Send the Set Filter File Command
/SCX_CPU1_DS_SetFilterFile MessageID=timeFileMsgID ParamIndex=0 FileIndex=timeFileEntry
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed (1004;5008) - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_FILE_CMD_EID,"."
  ut_setrequirements DS_1004, "F"
  ut_setrequirements DS_5008, "F"
endif
...
write ";***********************************************************************"
;; Dump the Packet Filter Table
s get_tbl_to_cvt (ramDir,filterTblName,"A","cpu1_filtertbl2133",hostCPU,filterTblPktId)

;; Verify the table contents reflect the commanded changes
if (SCX_CPU1_DS_PF_TBL[filterEntry].FilterParams[0].Index = timeFileEntry) then
...
;; Send the command
/SCX_CPU1_TST_DS_SENDMESSAGE MsgID=timeFileMsgID MsgType=1 Pattern=x'FF'
...
if (SCX_CPU1_DS_FileState[timeFileEntry].FileName <> "") then
  fileName = SCX_CPU1_DS_FileState[timeFileEntry].FileName
...
cmdCtr = SCX_CPU1_DS_CMDPC + 1
;; Send the Close commands
/SCX_CPU1_DS_CloseFile FileIndex=timeFileEntry
...
for i = 0 to DS_PACKETS_IN_FILTER_TABLE-1 do
  if (SCX_CPU1_DS_PF_TBL[i].MessageID <> seqMsgID) AND ;;
     (SCX_CPU1_DS_PF_TBL[i].MessageID <> timeMsgID) AND ;;
     (SCX_CPU1_DS_PF_TBL[i].MessageID <> oneMsgID) AND ;;
     (SCX_CPU1_DS_PF_TBL[i].MessageID <> xMsgID AND foundEntry = FALSE) AND ;;
     (SCX_CPU1_DS_PF_TBL[i].FilterParams[0].FilterType = DS_BY_TIME) then
    filterEntry = i
    foundEntry = TRUE
  endif
enddo
...
local seqFileMsgID = SCX_CPU1_DS_PF_TBL[filterEntry].MessageID
...
cmdCtr = SCX_CPU1_DS_CMDPC + 1
;; Send the Set Filter File Command
/SCX_CPU1_DS_SetFilterFile MessageID=seqFileMsgID ParamIndex=0 FileIndex=seqFileEntry
...
write ";***********************************************************************"
;; Dump the Packet Filter Table
s get_tbl_to_cvt (ramDir,filterTblName,"A","cpu1_filtertbl2163",hostCPU,filterTblPktId)

;; Verify the table contents reflect the commanded changes
if (SCX_CPU1_DS_PF_TBL[filterEntry].FilterParams[0].Index = seqFileEntry) then
  write "<*> Passed (5008) - Parameter changes are reflected in the table."
  ut_setrequirements DS_5008, "P"
else  
  write "<!> Failed (5008) - Expected parameter changes were not set in the Table entry."
...
;; Send the command
/SCX_CPU1_TST_DS_SENDMESSAGE MsgID=seqFileMsgID MsgType=1 Pattern=x'55'
...
if (SCX_CPU1_DS_FileState[seqFileEntry].FileName <> "") then
  fileName = SCX_CPU1_DS_FileState[seqFileEntry].FileName
...
for i = 0 to DS_DEST_FILE_CNT-1 do
  if (i <> seqFileEntry) AND ;;
     (SCX_CPU1_DS_DF_TBL[i].FileNameType = DS_BY_COUNT) AND ;;
...
cmdCtr = SCX_CPU1_DS_CMDPC + 1
;; Send the Set Filter File Command
/SCX_CPU1_DS_SetFilterFile MessageID=seqFileMsgID ParamIndex=0 FileIndex=newFileEntry
...
write ";***********************************************************************"
;; Dump the Packet Filter Table
s get_tbl_to_cvt (ramDir,filterTblName,"A","cpu1_filtertbl32",hostCPU,filterTblPktId)

;; Verify the table contents reflect the commanded changes
if (SCX_CPU1_DS_PF_TBL[filterEntry].FilterParams[0].Index = newFileEntry) then
  write "<*> Passed (5008) - Parameter changes are reflected in the table."
  ut_setrequirements DS_5008, "P"
else  
  write "<!> Failed (5008) - Expected parameter changes were not set in the Table entry."
...
;; Send the command
/SCX_CPU1_TST_DS_SENDMESSAGE MsgID=seqFileMsgID MsgType=1 Pattern=x'AA'
...
;; Verify that the file was created
fileName = ""
if (SCX_CPU1_DS_FileState[newFileEntry].FileName <> "") then
  fileName = SCX_CPU1_DS_FileState[newFileEntry].FileName
  write "<*> Passed (2002.1;3000) - A file was created with name '",fileName,"'"
  ut_setrequirements DS_20021, "P"
  ut_setrequirements DS_3000, "P"
else
...
cmdCtr = SCX_CPU1_DS_CMDPC + 1
;; Send the Close commands
/SCX_CPU1_DS_CloseFile FileIndex=newFileEntry
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {DSAppName}, DS_FILE_CMD_ERR_EID, "ERROR", 1
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1005) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",DS_FILE_CMD_ERR_EID, "."
  ut_setrequirements DS_1005, "F"
endif
...
;; Send the Set Filter File Command
/SCX_CPU1_DS_SetFilterFile MessageID=x'F345' ParamIndex=0 FileIndex=newFileEntry
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed (1005) - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_FILE_CMD_ERR_EID,"."
  ut_setrequirements DS_1005, "F"
endif
...
errcnt = SCX_CPU1_DS_CMDEC + 1
;; Send the Set Filter File Command
/SCX_CPU1_DS_SetFilterFile MessageID=seqFileMsgID ParamIndex=DS_FILTERS_PER_PACKET FileIndex=newFileEntry
...
errcnt = SCX_CPU1_DS_CMDEC + 1
;; Send the Set Filter File Command
/SCX_CPU1_DS_SetFilterFile MessageID=seqFileMsgID ParamIndex=0 FileIndex=DS_DEST_FILE_CNT
...
local existingFilterType = SCX_CPU1_DS_PF_TBL[filterEntry].FilterParams[0].FilterType
...
seqFileMsgID = SCX_CPU1_DS_PF_TBL[filterEntry].MessageID

;; Setup to capture the event
ut_setupevents "SCX", "CPU1", {DSAppName}, DS_FTYPE_CMD_EID, "DEBUG", 1

cmdCtr = SCX_CPU1_DS_CMDPC + 1
;; Send the Set Filter Type Command
/SCX_CPU1_DS_SetFilterType MessageID=seqFileMsgID, ParamIndex=0, FilterType=newFilterType 
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed (1004;5009) - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_FTYPE_CMD_EID,"."
  ut_setrequirements DS_1004, "F"
  ut_setrequirements DS_5009, "F"
endif
...
write ";***********************************************************************"
;; Dump the Packet Filter Table
s get_tbl_to_cvt (ramDir,filterTblName,"A","cpu1_filtertbl37",hostCPU,filterTblPktId)

;; Verify the table contents reflect the commanded changes
if (SCX_CPU1_DS_PF_TBL[filterEntry].FilterParams[0].FilterType = newFilterType) then
...
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {DSAppName}, DS_FTYPE_CMD_ERR_EID, "ERROR", 1
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1005) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",DS_FTYPE_CMD_ERR_EID, "."
  ut_setrequirements DS_1005, "F"
endif
...
errcnt = SCX_CPU1_DS_CMDEC + 1
;; Send the Set Filter Type Command
/SCX_CPU1_DS_SetFilterType MessageID=x'1345' ParamIndex=0 FilterType=DS_BY_COUNT
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed (1005) - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_FTYPE_CMD_ERR_EID,"."
  ut_setrequirements DS_1005, "F"
endif
...
errcnt = SCX_CPU1_DS_CMDEC + 1
;; Send the Set Filter Type Command
/SCX_CPU1_DS_SetFilterType MessageID=seqFileMsgID ParamIndex=DS_FILTERS_PER_PACKET FilterType=DS_BY_COUNT
...
errcnt = SCX_CPU1_DS_CMDEC + 1
;; Send the Set Filter Type Command
/SCX_CPU1_DS_SetFilterType MessageID=seqFileMsgID ParamIndex=0 FilterType=3
...
local existingNVal = SCX_CPU1_DS_PF_TBL[filterEntry].FilterParams[0].N_Value
local existingXVal = SCX_CPU1_DS_PF_TBL[filterEntry].FilterParams[0].X_Value
local existingOVal = SCX_CPU1_DS_PF_TBL[filterEntry].FilterParams[0].O_Value
...
/SCX_CPU1_DS_SetFilterParams MessageID=seqFileMsgID ParamIndex=0 N_Value=newNVal X_Value=newXVal O_Value=newOVal
...
write ";***********************************************************************"
;; Dump the Packet Filter Table
s get_tbl_to_cvt (ramDir,filterTblName,"A","cpu1_filtertbl312",hostCPU,filterTblPktId)

;; Verify the table contents reflect the commanded changes
if (SCX_CPU1_DS_PF_TBL[filterEntry].FilterParams[0].N_Value = newNVal) AND ;;
   (SCX_CPU1_DS_PF_TBL[filterEntry].FilterParams[0].X_Value = newXVal) AND ;;
   (SCX_CPU1_DS_PF_TBL[filterEntry].FilterParams[0].O_Value = newOVal) then
  write "<*> Passed (5010) - Parameter changes are reflected in the table."
  ut_setrequirements DS_5010, "P"
else  
  write "<!> Failed (5010) - Expected parameter changes were not set in the Table entry."
...
existingFilterType = SCX_CPU1_DS_PF_TBL[filterEntry].FilterParams[0].FilterType
...
if (existingFilterType = DS_BY_TIME) then
  ...
  cmdCtr = SCX_CPU1_DS_CMDPC + 1
  ;; Send the Set Filter Type Command
  /SCX_CPU1_DS_SetFilterType MessageID=seqFileMsgID, ParamIndex=0, FilterType=DS_BY_COUNT
...
expPassedPkts = SCX_CPU1_DS_PassedPktCnt + newNVal

;; Send the command
for i = 1 to newXVal do
  ;; Set the data pattern to send
  dataPattern = i
  /SCX_CPU1_TST_DS_SENDMESSAGE MsgID=seqFileMsgID MsgType=1 Pattern=dataPattern
  wait 5
enddo

;; Check for the event message
if (SCX_CPU1_find_event[1].num_found_messages = newXVal) then
  write "<*> Passed - Expected Event Msgs ",TST_DS_MSG1_SENT_INF_EID," rcv'd."
else
  write "<!> Failed - Expected ",newXVal," event messages. Rcv'd ",SCX_CPU1_find_event[1].num_found_messages,"."
endif
...
write ";  length."
write ";*********************************************************************"
ut_setupevents "SCX", "CPU1", {DSAppName}, DS_PARMS_CMD_ERR_EID, "ERROR", 1
...
if (SCX_CPU1_find_event[1].num_found_messages = 1) THEN
  ...
  write "<!> Failed (1005) - Event message ",SCX_CPU1_evs_eventid," received. Expected Event message ",DS_PARMS_CMD_ERR_EID, "."
  ut_setrequirements DS_1005, "F"
endif
...
errcnt = SCX_CPU1_DS_CMDEC + 1
;; Send the Set Filter Type Command
/SCX_CPU1_DS_SetFilterParams MessageID=x'ABCD' ParamIndex=0 N_Value=newNVal X_Value=newXVal O_Value=newOVal
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed (1005) - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_PARMS_CMD_ERR_EID,"."
  ut_setrequirements DS_1005, "F"
endif
...
errcnt = SCX_CPU1_DS_CMDEC + 1
;; Send the Set Filter Type Command
/SCX_CPU1_DS_SetFilterParams MessageID=x'ABCD' ParamIndex=DS_FILTERS_PER_PACKET  N_Value=newNVal X_Value=newXVal O_Value=newOVal
...
errcnt = SCX_CPU1_DS_CMDEC + 1
;; Send the Set Filter Type Command
/SCX_CPU1_DS_SetFilterParams MessageID=seqFileMsgID ParamIndex=0  N_Value=5 X_Value=3 O_Value=0
...
errcnt = SCX_CPU1_DS_CMDEC + 1
;; Send the Set Filter Type Command
/SCX_CPU1_DS_SetFilterParams MessageID=seqFileMsgID ParamIndex=0  N_Value=1 X_Value=3 O_Value=4
...
if (existingFilterType = DS_BY_COUNT) then
  ...
  cmdCtr = SCX_CPU1_DS_CMDPC + 1
  ;; Send the Set Filter Type Command
  /SCX_CPU1_DS_SetFilterType MessageID=seqFileMsgID, ParamIndex=0, FilterType=DS_BY_TIME
...
for i = 1 to newXVal do
  ...
  ;; Set the data pattern to send
  dataPattern = i
  /SCX_CPU1_TST_DS_SENDTIMEMESSAGE MsgID=seqFileMsgID MsgType=1 Pattern=dataPattern Seconds=0 SubSecs=subSecs
  wait 5
enddo
...
write ";*********************************************************************"
;; Setup the event capture
ut_setupevents "SCX", "CPU1", {DSAppName}, DS_ADD_MID_CMD_EID, "DEBUG", 1

cmdCtr = SCX_CPU1_DS_CMDPC + 1
;; Send the command
/SCX_CPU1_DS_AddMID MessageID=addMID
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed (1004) - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_ADD_MID_CMD_EID,"."
  ut_setrequirements DS_1004, "F"
endif
...
write ";*********************************************************************"
;; Dump the Packet Filter Table
s get_tbl_to_cvt (ramDir,filterTblName,"A","cpu1_filtertbl42",hostCPU,filterTblPktId)

local foundMID = FALSE
;; Verify the table contains the added MessgeID
for i = 0 to DS_PACKETS_IN_FILTER_TABLE-1 do
  if (SCX_CPU1_DS_PF_TBL[i].MessageID = addMID) then
...
write ";*********************************************************************"
;; Setup the event capture
ut_setupevents "SCX", "CPU1", {DSAppName}, DS_ADD_MID_CMD_ERR_EID, "ERROR", 1

errcnt = SCX_CPU1_DS_CMDEC + 1
;; Send the command
/SCX_CPU1_DS_AddMID MessageID=0
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed (1005;5016.1) - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_ADD_MID_CMD_ERR_EID,"."
  ut_setrequirements DS_1005, "F"
  ut_setrequirements DS_50161, "F"
endif
...
errcnt = SCX_CPU1_DS_CMDEC + 1
;; Send the command
/SCX_CPU1_DS_AddMID MessageID=seqMsgID 
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed (1005;5016.3) - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_ADD_MID_CMD_ERR_EID,"."
  ut_setrequirements DS_1005, "F"
  ut_setrequirements DS_50163, "F"
endif
...
s ftp_file("CF:0", "na", filterFileName, hostCPU, "R")
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed (1005;5016.2) - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_ADD_MID_CMD_ERR_EID,"."
  ut_setrequirements DS_1005, "F"
  ut_setrequirements DS_50162, "F"
endif
...
;; Send the command
/SCX_CPU1_DS_AddMID MessageID=fullMID 
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed (1005;5016.4) - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_ADD_MID_CMD_ERR_EID,"."
  ut_setrequirements DS_1005, "F"
...
write ";*********************************************************************"
write ";  End procedure SCX_CPU1_ds_filter"
write ";*********************************************************************"
ENDPROC
