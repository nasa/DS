...
write "==> Default File Table filename   = '",destFileName,"'"
...
s get_file_to_cvt (ramDir, "cfe_sb_route.dat", "cpu1_sb_route.dat", hostCPU)
wait 5
...
for i = 0 to DS_PACKETS_IN_FILTER_TABLE-1 do
  filterMsgID = SCX_CPU1_DS_PF_TBL[i].MessageID
  if (filterMsgID <> 0) then
    filterEntries = filterEntries + 1
    write "== Looking for msgID = ", %hex(filterMsgId,4)
    for sbIndex = 1 to 16320 do
      if  (SCX_CPU1_SB_RouteEntry[sbIndex].SB_AppName = DSAppName) AND ;;
          (SCX_CPU1_SB_RouteEntry[sbIndex].SB_MsgID = filterMsgID) then
        foundSubscription = foundSubscription + 1
        break
      elseif (SCX_CPU1_SB_RouteEntry[sbIndex].SB_AppName = "") then
        break
      endif
    enddo
  endif
...
if (DS_DEF_ENABLE_STATE = 1) then
  if (p@SCX_CPU1_DS_AppEnaState = "Enabled") then
    ...
    write "<!> Failed (9008) - DS Application State is NOT enabled as expected. State = '",p@SCX_CPU1_DS_AppEnaState,"'"
  ...
  if (p@SCX_CPU1_DS_AppEnaState = "Disabled") then
    ...
    write "<!> Failed (9008) - DS Application State is NOT disabled as expected. State = '",p@SCX_CPU1_DS_AppEnaState,"'"
    ut_setrequirements DS_9008, "F"
  endif
endif
...
local msgIDs[3]
for i = 0 to DS_PACKETS_IN_FILTER_TABLE-1 do
  if (SCX_CPU1_DS_PF_TBL[i].MessageID <> 0) then
    msgIDs[entryCount] = SCX_CPU1_DS_PF_TBL[i].MessageID
...
for i = 1 to 3 do
  ;; Send the command to
  /SCX_CPU1_TST_DS_SENDMESSAGE MsgID=msgIDs[i] MsgType=1 Pattern=x'A5'
  wait 5
enddo
...
if (SCX_CPU1_find_event[1].num_found_messages = 3) then
  write "<*> Passed - Rcv'd the expected number of Send events."
else
  write "<!> Failed - Expected 3 Send events. Rcv'd ",SCX_CPU1_find_event[1].num_found_messages
endif
...
local fileName
for i = 0 to DS_DEST_FILE_CNT-1 do
  if (SCX_CPU1_DS_FileState[i].FileName <> "") then
    fileName = SCX_CPU1_DS_FileState[i].FileName
    ...
    if (p@SCX_CPU1_DS_DF_TBL[i].FileNameType = "Count") then
      seqCount = seqCount + 1
...
local newNVal = SCX_CPU1_DS_PF_TBL[3].FilterParams[0].N_Value + 1
local newXVal = SCX_CPU1_DS_PF_TBL[3].FilterParams[0].X_Value + 1
local newOVal = SCX_CPU1_DS_PF_TBL[3].FilterParams[0].O_Value + 1
...
/SCX_CPU1_DS_SetFilterParams MessageID=SCX_CPU1_DS_PF_TBL[3].MessageID ParamIndex=0 N_Value=newNVal X_Value=newXVal O_Value=newOVal
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_PARMS_CMD_EID,"."
endif
...
cmdCtr = SCX_CPU1_DS_CMDPC + 1
;; Send the command
/SCX_CPU1_DS_SetPath FileIndex=1 PathName=newPath
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_PATH_CMD_EID,"."
endif
...
if (SCX_CPU1_DS_PF_TBL[3].FilterParams[0].N_Value = newNVal) AND ;;
   (SCX_CPU1_DS_PF_TBL[3].FilterParams[0].X_Value = newXVal) AND ;;
   (SCX_CPU1_DS_PF_TBL[3].FilterParams[0].O_Value = newOVal) then
...
s get_tbl_to_cvt (ramDir,fileTblName,"A","cpu1_filetbl25",hostCPU,fileTblPktId)

;; Verify the table contains the change made to the 2nd entry
if (p@SCX_CPU1_DS_DF_TBL[1].Pathname = newPath) then
...
if (DS_CDS_ENABLE_STATE = 1) then
  if (p@SCX_CPU1_DS_AppEnaState = expectedState) then
    write "<*> Passed (9003) - DS Application State is set as expected."
    ut_setrequirements DS_9003, "P"
  else
    write "<!> Failed (9003) - DS Application State is NOT as expected. State = '",p@SCX_CPU1_DS_AppEnaState,"'. Expected '",expectedState,"'"
    ut_setrequirements DS_9003, "F"
  endif
else
  if (DS_DEF_ENABLE_STATE = 1) then
...
for i = 1 to openCount do
  if (SCX_CPU1_DS_FileState[openFiles[i]].FileName = "") then
...
for i = 1 to 3 do
  ;; Send the command to
  /SCX_CPU1_TST_DS_SENDMESSAGE MsgID=msgIDs[i] MsgType=1 Pattern=x'BB'
  wait 5
enddo
...
/SCX_CPU1_DS_SetFilterParams MessageID=SCX_CPU1_DS_PF_TBL[3].MessageID ParamIndex=1 N_Value=newNVal X_Value=newXVal O_Value=newOVal
...
cmdCtr = SCX_CPU1_DS_CMDPC + 1
;; Send the command
/SCX_CPU1_DS_SetBasename FileIndex=1 BaseName="newSeq"
...
if (UT_TW_Status = UT_Success) then
  ...
  write "<!> Failed - Event message ", SCX_CPU1_evs_eventid," rcv'd. Expected Event Msg ",DS_BASE_CMD_EID,"."
endif
...
;; Verify the table contents reflect the commanded changes
if (SCX_CPU1_DS_PF_TBL[3].FilterParams[1].N_Value = newNVal) AND ;;
   (SCX_CPU1_DS_PF_TBL[3].FilterParams[1].X_Value = newXVal) AND ;;
   (SCX_CPU1_DS_PF_TBL[3].FilterParams[1].O_Value = newOVal) then
  write "<*> Passed (9003) - Parameter changes are reflected in the table."
  ut_setrequirements DS_9003, "P"
else
  write "<!> Failed (9003) - Expected parameter changes were not set in the Table entry."
...
;; Verify the Destination File Table changes
s get_tbl_to_cvt (ramDir,fileTblName,"A","cpu1_filetbl332",hostCPU,fileTblPktId)

;; Verify the table contains the change made to the 2nd entry
if (p@SCX_CPU1_DS_DF_TBL[1].Basename = "newSeq") then
...
for i = 1 to 3 do
  ;; Send the command to
  /SCX_CPU1_TST_DS_SENDMESSAGE MsgID=msgIDs[i] MsgType=1 Pattern=x'55'
  wait 5
enddo
...
write ";***********************************************************************"
write ";  Step 4.2: Perform a Power-On Reset on CPU1."
write ";***********************************************************************"
/SCX_CPU1_ES_POWERONRESET
wait 10
...
ut_setupevents "SCX","CPU1","CFE_TBL",CFE_TBL_FILE_ACCESS_ERR_EID, "ERROR", 3
...
write ";***********************************************************************"
s ftp_file("CF:0", "na", destFileName, hostCPU, "R")
wait 30

write ";***********************************************************************"
write ";  Step 4.5: Perform a Power-On Reset on CPU1."
write ";***********************************************************************"
/SCX_CPU1_ES_POWERONRESET
wait 10
...
write ";***********************************************************************"
write ";  Step 4.8: Perform a Power-On Reset on CPU1."
write ";***********************************************************************"
/SCX_CPU1_ES_POWERONRESET
wait 10
...
write ";***********************************************************************"
write ";  Step 4.11: Perform a Power-On Reset on CPU1."
write ";***********************************************************************"
/SCX_CPU1_ES_POWERONRESET
wait 10
...
write ";*********************************************************************"
local addval, filtHandle, fileHandle
page SCX_CPU1_ES_CDS_REGISTRY

addval = 16 + CFE_ES_RESET_AREA_SIZE + (CFE_ES_RAM_DISK_SECTOR_SIZE * CFE_ES_RAM_DISK_NUM_SECTORS) + 20 + 4

; Dump the CDS Registry
s get_file_to_cvt (ramDir, "cfe_cds_reg.log", "cpu1_cds_reg.log", hostCPU)
wait 5

; Loop for the DS Tables
for i = 1 to CFE_PLATFORM_ES_CDS_MAX_NUM_ENTRIES do
  if (SCX_CPU1_ES_CDSREG[i].CDSNAME <> "") then
    write "CDS Name = ",SCX_CPU1_ES_CDSREG[i].CDSNAME
    write "CDS Handle = ",SCX_CPU1_ES_CDSREG[i].CDSHANDLE
    if (SCX_CPU1_ES_CDSREG[i].CDSNAME = "DS.FILTER_TBL") then
      filtHandle = SCX_CPU1_ES_CDSREG[i].CDSHANDLE
    elseif (SCX_CPU1_ES_CDSREG[i].CDSNAME = "DS.FILE_TBL") then
      fileHandle = SCX_CPU1_ES_CDSREG[i].CDSHANDLE
    endif
  else
    break;
  endif
...
write "     3. Click the Refresh button on the SCX_CPU1_ES_CDS_REGISTRY page"
write "     4. Note the CDS ""Handle"" for DS.FILTER_TBL on that page"
write "     5. Add the ", filtHandle, " to the sum calculated in Step 2."
...
write ";*********************************************************************"
write ";  Step 5.2: Perform a Processor Reset on CPU1."
write ";*********************************************************************"
/SCX_CPU1_ES_PROCESSORRESET
wait 10
...
s get_tbl_to_cvt (ramDir,filterTblName,"A","cpu1_filtertbl53",hostCPU,filterTblPktId)

;; Verify the table contents reflect the original contents
if (SCX_CPU1_DS_PF_TBL[3].FilterParams[1].N_Value = 0) AND ;;
   (SCX_CPU1_DS_PF_TBL[3].FilterParams[1].X_Value = 0) AND ;;
   (SCX_CPU1_DS_PF_TBL[3].FilterParams[1].O_Value = 0) then
...
write "     4. Add the ", fileHandle, " to the sum calculated in Step 2."
...
write ";*********************************************************************"
write ";  Step 5.6: Perform a Processor Reset on CPU1."
write ";*********************************************************************"
/SCX_CPU1_ES_PROCESSORRESET
wait 10
...
s get_tbl_to_cvt (ramDir,fileTblName,"A","cpu1_filetbl57",hostCPU,fileTblPktId)

;; Verify the table contains the change made to the 2nd entry
if (p@SCX_CPU1_DS_DF_TBL[1].Pathname <> "/cf/") AND ;;
   (p@SCX_CPU1_DS_DF_TBL[1].Basename <> "newSeq") then
  write "<*> Passed - The original parameter values are reflected in the table."
else
...
for i = 1 to 5 do
  ...
  write ";***********************************************************************"
  ;; Load the table
  ut_setupevents "SCX", "CPU1", "CFE_TBL",CFE_TBL_FILE_LOADED_INF_EID,"INFO", 1
  ...
  ut_setupevents "SCX","CPU1","CFE_TBL",CFE_TBL_VALIDATION_ERR_EID, "ERROR", 2
  ut_setupevents "SCX","CPU1",{DSAppName},DS_FLT_TBL_ERR_EID, "ERROR", 3
  ut_setupevents "SCX","CPU1",{DSAppName},DS_FLT_TBL_EID, "INFO", 4
  ...
  if (SCX_CPU1_find_event[3].num_found_messages > 0) then
  ...
  ;; Verify the Validation Error event was generated by DS
  if (SCX_CPU1_find_event[4].num_found_messages > 0) then
  ...
  write ";***********************************************************************"
  ut_setupevents "SCX","CPU1","CFE_TBL",CFE_TBL_LOAD_ABORT_INF_EID,"INFO", 1

  cmdCtr = SCX_CPU1_TBL_CMDPC + 1

  /SCX_CPU1_TBL_LOADABORT ABTABLENAME=filterTblName
...
for i = 1 to 9 do
  ...
  ut_setupevents "SCX","CPU1","CFE_TBL",CFE_TBL_VAL_REQ_MADE_INF_EID, "DEBUG", 1
  ut_setupevents "SCX","CPU1","CFE_TBL",CFE_TBL_VALIDATION_ERR_EID, "ERROR", 2
  ut_setupevents "SCX","CPU1",{DSAppName},DS_FIL_TBL_ERR_EID, "ERROR", 3
  ut_setupevents "SCX","CPU1",{DSAppName},DS_FIL_TBL_EID, "INFO", 4
  ...
  /SCX_CPU1_TBL_LOADABORT ABTABLENAME=fileTblName
...
write ";*********************************************************************"
write ";  End procedure SCX_CPU1_ds_resetcds"
write ";*********************************************************************"
ENDPROC
